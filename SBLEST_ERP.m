...
Loss_old = 0;
threshold = 0.05;
%% reshape the dataset into two dimension and compute initial Sigma_y
...
[SF_all, D, TF_all] = svd(W); 
alpha_all = diag(D); 

%% Select L pairs of Spatial filters, Temporal filter and alpha.
d = abs(diag(D)); d_max = max(d); 
w_norm = d/d_max; % normalize singular Values of W by the maximum eigenvalue
index = find(w_norm > threshold); % find index of selected TF,SF,alpha by pre-defined threshold,.e.g., 0.05
SF = SF_all(:,index);
TF = TF_all(:,index);
alpha = alpha_all(index);
% select Spatial filters, Temporal filter and alpha.
end
